...
JedWebpackPlugin.prototype.apply = function(compiler) {
  ...
  compiler.plugin('compilation', function(compilation, params) {
    compilation.dependencyFactories.set(PrefetchDependency, params.normalModuleFactory);
    compilation.dependencyFactories.set(ConstDependency, new NullFactory());
    compilation.dependencyTemplates.set(ConstDependency, new ConstDependency.Template());

    // Add jed module to chunk
    ...
    compilation.mainTemplate.plugin('require-extensions', function(source, chunk, hash) {
      var jedModule = chunk.modules.find(function(module) {
        return module.rawRequest === 'jed';
      });
  ...
  if (that.defineJedIdentifier) {
    compiler.parser.plugin('expression ' + that.defineJedIdentifier, function(expr) {
  ...
  jedFunctions.forEach(function(func) {
    compiler.parser.plugin('call ' + func, function(expr) {
